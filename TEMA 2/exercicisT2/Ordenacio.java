...
public class Ordenacio {
    ...
    public static void selDirectaRec(int[] v, int pos) {
        if (pos < v.length - 1) {
            // calcular la posició del mínim de v[pos..v.length-1] 
            int pMin = pos;
            for (int j = pos + 1; j < v.length; j++) { 
                if (v[j] < v[pMin]) { pMin = j; }
                // en pMin està la posició del mínim de v[pos..j] 
            }
            // en pMin està la posició del mínim de v[pos..v.length-1]
            // intercanviar v[pos] amb v[pMin]
            int aux = v[pMin]; 
            v[pMin] = v[pos]; 
            v[pos] = aux;       // v[0..pos] ordenat 
            // crida recursiva per ordenar fins pos+1
    ...
    public static void insDirecta(int[] v) {
        ...
        for (int i = 1; i <= v.length - 1; i++) {
            ...
            int j = i - 1;  // fi de la part ordenada
    ...
    public static void bambollaAmbFlag(int[] v) {
        ...
        // cerca ascendent de v[0..v.length-2]
        for (int i = 0; i < v.length - 1 && !ordenat; i++) {
            int cont = 0; 
            // recorregut descendent de v[i+1..v.length-1]
            // comptant el numero d'intercanvis
            for (int j = v.length - 1; j > i; j--) { 
                if (v[j - 1] > v[j]) {
            ...
            // en la iteració i, el i+1-èsim mínim queda  
            // situat en la posició i de l’array i si no hi 
            // ha hagut cap intervanvi es que ja esta ordenat
    ...
    public static void bambollaM(int[] v, int m) {
        // recorregut ascendent de v[0..m-1]
        for (int i = 0; i < m; i++) {
            // recorregut descendent de v[i+1..v.length-1]
            for (int j = v.length - 1; j > i; j--) { 
                // comparar parells d’elements consecutius
